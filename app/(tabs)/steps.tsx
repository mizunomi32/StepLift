...
import { calculateCalories, calculateDistance } from '@/lib/utils/step-calculations';

export default function StepsScreen() {
  const { todaySteps, dailyGoal, weeklyRecords, startTracking, loadWeeklyRecords } =
    useStepsStore();

  useEffect(() => {
    startTracking();
    loadWeeklyRecords();
  }, [loadWeeklyRecords, startTracking]);

  const isAchieved = todaySteps >= dailyGoal;
  const todayRecord = weeklyRecords.find((r) => r.date === new Date().toISOString().split('T')[0]);
  ...
  return (
    <ScrollView testID="steps-screen" className="flex-1 bg-black" contentContainerClassName="p-6">
      {/* 円形プログレスリング */}
      ...
      <View className="mb-6">
        <Link href="/steps/goal-setting" asChild>
          ...
          >
            ...
            <Text className="text-white font-semibold">{dailyGoal.toLocaleString('ja-JP')}歩</Text>
            {isAchieved && <Text className="text-green-500 ml-3 font-semibold">✓ 達成!</Text>}
